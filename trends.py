...
def tweet_location(tweet):
    ...
    return make_position(tweet['latitude'],tweet['longtitude'])
...
def find_centroid(polygon):
    ...
    def area(polygon):
        index = 0
        t_area = 0
        while index+1 < len(polygon):
            t_area = (t_area + 0.5*((polygon[index][0])*(polygon[index+1][1]) - (polygon[index+1][0])*(polygon[index][1])))
            index = index+1
        return t_area
    ...
    if area(polygon) == 0:
        ...
        def cx(polygon):
            ...
            totalcx = 0
            while index+1 < len(polygon):
                totalcx = totalcx + (((polygon[index][0])+(polygon[index+1][0]))*((polygon[index][0])*(polygon[index+1][1])-(polygon[index+1][0])*(polygon[index][1])))
                index = index + 1
            return totalcx 
        ...
        def cy(polygon):
            ...
            totalcy = 0
            while index+1 < len(polygon):
                totalcy = totalcy + (((polygon[index][1])+(polygon[index+1][1]))*((polygon[index][0])*(polygon[index+1][1])-(polygon[index+1][0])*(polygon[index][1])))
                index = index + 1
            return totalcy
        return (1/(6*(area(polygon)))*cx(polygon),1/(6*(area(polygon)))*cy(polygon),abs(area(polygon)))
...
def find_center(shapes):
    ...
    def area(polygon):
        ...
        t2_area = 0
        while index+1 < len(polygon):
            t2_area = (t2_area + 0.5*((polygon[index][0])*(polygon[index+1][1]) - (polygon[index+1][0])*(polygon[index][1])))
            index = index + 1
        return t2_area
    index = 0
    total_area = 0
    centroid_and_area_x = 0
    centroid_and_area_y = 0
    while index < len(shapes):
        total_area = total_area + area(shapes[index])
        centroid_and_area_x = centroid_and_area_x + (find_centroid(shapes[index][0]))*area(shape[index])
        centroid_and_area_y = centroid_and_area_y + (find_centroid(shapes[index][1]))*area(shape[index])
        index = index + 1
    return (((centroid_and_area_x)/total_area), ((centroid_and_area_y)/total_area))
...
iterate through us_states and call find_center for each to get state_centers in group_tweets_by_state ?

state_centers = {key['state']:find_center(state) for state in us_states}
...
def find_closest_state(tweet, state_centers):
    ...
    a = {n:geo_distance(tweet_location(tweet),us_centers[n]) for n in us_centers}
    closest_state = min(a, key=a.get)
...
def group_tweets_by_state(tweets):
    ...
    """
    postal_code_dict = {postal_code:find_closest_state(tweet_location(tweet),us_centers) for tweet in tweets}
    tweets_by_state = {statetweet:key[postal_code_dict]==postal_code for tweet in postal_code_dict}
...
def calculate_average_sentiments(tweets_by_state):
    ...
    for n, s in us_states.items():
        sum = 0.0
        terms = 0
        for tweet in tweets_by_state[n]:
            ave_sent = 0.0
            y = analyze_tweet_sentiment(tweet)
            if (type(y)==float):
                ave_sent = y
                terms = terms + 1
            sum = sum + ave_sent
        if (sum != 0) and (terms != 0):
            averaged_state_sentiments[n] = sum/terms
